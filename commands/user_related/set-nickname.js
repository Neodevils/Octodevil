module.exports = {
	...
	$onlyIf[$charCount[$interactionData[options.data[0].value]]<9;{ "embeds": "{newEmbed:{description:$nonEscape[$customEmoji[kaeruTag]] *Name should not be longer than 9 chars.*}{color: $getVar[invisibleHex]}}",  "ephemeral": true, "options":{ "interaction" : true } }]

	$onlyForChannels[926099952545972244;{"content":"➜ <#926099952545972244>", "embeds": "{newEmbed:{description:<#$channelID> is not the right channel, use the channel on above with showed arrow.}{color:$getVar[invisible Hex]}}", "ephemeral": true, "options":{ "interaction" : true } }]
	`
}